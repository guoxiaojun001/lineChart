...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		int[] yValues = new int[]{20, 18, 19, 18, 45, 40, 33, 32, 46, 53, 50, 42};
		for(int i = 8; i <= 19; i++) {
			data.addPoint(i, yValues[i-8]);
			data.addXValue(i, i + "ÔÂ");
		}
		chart.addData(data);
		
		ChartData data2 = new ChartData(ChartData.LINE_COLOR_BLACK);
		int[] yValues2 = new int[]{0, 5, 9, 23, 15, 35, 45, 50, 41, 45, 32, 24};
		for(int i = 8; i <= 19; i++) {
			data2.addPoint(i, yValues2[i-8]);
			//data2.addXValue(i, i + "ÔÂ");
		}
		chart.addData(data2);
		
		
//		ChartData data3 = new ChartData(ChartData.LINE_COLOR_RED);
//		int[] yValues3 = new int[]{7, 7, 7, 3, 40, 5, 15, 10, 21, 25, 22, 44};
//		for(int i = 8; i <= 19; i++) {
//			data3.addPoint(i, yValues3[i-8]);
//			data3.addXValue(i, i + ":00");
//		}
//		chart.addData(data3);
	}
